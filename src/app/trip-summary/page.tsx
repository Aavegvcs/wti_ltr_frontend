...
export default function TripSummaryPage() {
  ...
  useEffect(() => {
    const data = JSON.parse(sessionStorage.getItem("tripEnd") || "null");
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center px-4 py-8">
      <div className="w-full max-w-md bg-white p-6 rounded-xl shadow-md">
        ...
        <div className="mt-6 space-y-4">
          <div>
            <p className="font-medium">Driver Signature:</p>
            <SignatureCanvas ref={driverSign} penColor="black" canvasProps={{ width: 300, height: 100, className: "border border-gray-400 rounded-md" }} />
          </div>
          <div>
            <p className="font-medium">Passenger Signature:</p>
            <SignatureCanvas ref={passengerSign} penColor="black" canvasProps={{ width: 300, height: 100, className: "border border-gray-400 rounded-md" }} />
          </div>

          <button onClick={handleSave} className="bg-blue-600 text-white w-full rounded-lg py-3 font-medium hover:bg-blue-700">
            Save Trip Sheet
          </button>
        </div>
      </div>
